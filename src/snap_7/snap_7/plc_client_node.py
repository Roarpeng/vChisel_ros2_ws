...
class PLCClientNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("plc_address", "192.168.50.228")
        self.declare_parameter("plc_rack", 0)
        self.declare_parameter("plc_slot", 1)
        self.declare_parameter(
            "db_number", 2120
        )  # Updated to match the working test script
        ...
        self.plc_address = (
            self.get_parameter("plc_address").get_parameter_value().string_value
        )
        self.plc_rack = (
            self.get_parameter("plc_rack").get_parameter_value().integer_value
        )
        self.plc_slot = (
            self.get_parameter("plc_slot").get_parameter_value().integer_value
        )
        self.db_number = (
            self.get_parameter("db_number").get_parameter_value().integer_value
        )
        self.db_start = (
            self.get_parameter("db_start").get_parameter_value().integer_value
        )
        self.poll_rate = float(
            self.get_parameter("poll_rate").get_parameter_value().double_value
        )
        self.simulate_plc = (
            self.get_parameter("simulate_plc").get_parameter_value().bool_value
        )

        if snap7 is None:
            self.get_logger().error(
                "python-snap7 is not installed. Install it with pip install snap7"
            )
            rclpy.shutdown()
            return
    ...
    def connect(self):
        try:
            ...
            self.get_logger().info(
                f"Connected to PLC at {self.plc_address} (rack={self.plc_rack}, slot={self.plc_slot})"
            )
        except Exception as e:
    ...
    def connect_with_retry(self, max_retries=3):
        ...
        while retry_count < max_retries:
            ...
            if retry_count < max_retries:
                self.get_logger().info(
                    f"Connect retry {retry_count}/{max_retries} in {retry_interval}s"
                )
        ...
        self.get_logger().warning(
            "Failed to connect to PLC after retries, continuing with limited functionality"
        )
        ...
        with self.lock:
            try:
                ...
                self.get_logger().debug(
                    f"Debug: Error reading DB: {e}"
                )  # Use debug level to reduce spam
    ...
    def write_real_to_db(self, offset, value):
        ...
        with self.lock:
            try:
                if self.simulate_plc:
                    self.get_logger().info(
                        f"[LOG] Simulated write REAL to DB{self.db_number} offset {offset}: {value}"
                    )
                    return True
                ...
                if not self.client.get_connected():
                    ...
                    if not self.connected:
                        self.get_logger().warning(
                            f"Cannot write to PLC at offset {offset}: not connected"
                        )
                        return False
                ...
                self.get_logger().info(
                    f"[LOG] Sending REAL to DB{self.db_number}[{offset}]: {value} (bytes: {data.hex()})"
                )

                # 写入DB
                self.client.db_write(self.db_number, offset, data)
                self.get_logger().debug(
                    f"Successfully wrote REAL to DB{self.db_number}[{offset}] = {value}"
                )
                return True
            except Exception as e:
                self.get_logger().error(
                    f"Failed to write REAL to DB{self.db_number}[{offset}]: {e}"
                )
                return False
    ...
    def write_integer_to_db(self, offset, value):
        ...
        with self.lock:
            try:
                if self.simulate_plc:
                    self.get_logger().info(
                        f"[LOG] Simulated write to DB{self.db_number} offset {offset}: {value}"
                    )
                    return True
                ...
                # 记录发送的数据日志，便于对比发送的数据和接收的数据是否一致
                self.get_logger().info(
                    f"[LOG] Sending integer to DB{self.db_number}[{offset}]: {value} (bytes: {data[0]:02X}{data[1]:02X})"
                )

                # 写入DB
                self.client.db_write(self.db_number, offset, data)
                self.get_logger().debug(
                    f"Successfully wrote to DB{self.db_number}[{offset}] = {value}"
                )
                return True
            except Exception as e:
                error_msg = str(e)
                ...
                if (
                    "CPU : Item not available" in error_msg
                    or "function refused by CPU" in error_msg
                ):
                    self.get_logger().warning(
                        f"PLC CPU refused write operation at DB{self.db_number} offset {offset}. This may be due to DB configuration or PLC security settings."
                    )
                    # Add more detailed logging about the attempted write
                    self.get_logger().info(
                        f"Attempted to write value {value} to DB{self.db_number} at offset {offset}"
                    )
                    self.get_logger().info(
                        'PLC Configuration Required: Ensure "Full Access" level and "PUT/GET Communication" are enabled in PLC security settings. Also verify "Optimized Block Access" is turned OFF in the DB properties in TIA Portal.'
                    )
                else:
    ...
    def write_db_area(self, data_bytes, start=64):
        with self.lock:
            try:
                if self.simulate_plc:
                    self.get_logger().info(
                        f"[LOG] Simulated write to DB{self.db_number} offset {start}: {data_bytes}"
                    )
                    return True
                ...
                if not self.client.get_connected():
                    ...
                    if not self.connected:
                        self.get_logger().warning(
                            f"Cannot write to PLC at offset {start}: not connected"
                        )
                        return False
                ...
                if not isinstance(data_bytes, (bytes, bytearray)):
                    self.get_logger().error(
                        f"Invalid data type for writing: {type(data_bytes)}"
                    )
                    return False
                ...
                # 记录发送的数据日志，便于对比发送的数据和接收的数据是否一致
                self.get_logger().info(
                    f"[LOG] Sending {len(data_bytes)} bytes to DB{self.db_number}[{start}]: {data_bytes.hex() if hasattr(data_bytes, 'hex') else bytes(data_bytes).hex()}"
                )
                ...
                if (
                    ...
                    self.get_logger().warning(
                        f"PLC CPU refused write operation at DB{self.db_number} offset {start}. This may be due to DB configuration or PLC security settings."
                    )
                    ...
                    self.get_logger().info(
                        f"Attempted to write {len(data_bytes)} bytes to DB{self.db_number} at offset {start}"
                    )
                    self.get_logger().info(
                        'PLC Configuration Required: Ensure "Full Access" level and "PUT/GET Communication" are enabled in PLC security settings. Also verify "Optimized Block Access" is turned OFF in the DB properties in TIA Portal.'
                    )
    ...
    def write_real_array_to_db(self, real_list, start=20):
        ...
        if not real_list:
            self.get_logger().warning(
                f"Empty REAL list provided for write at offset {start}"
            )
            return False
        ...
        if not self.client.get_connected():
            self.get_logger().error(
                f"Cannot write to PLC: not connected. Attempting to connect..."
            )
            self.connect()
            if not self.connected:
                self.get_logger().error(
                    f"Failed to connect to PLC, aborting write at offset {start}"
                )
                return False

        # 记录发送的数据日志，便于对比发送的数据和接收的数据是否一致
        self.get_logger().info(
            f"[LOG] Sending {len(real_list)} REAL values to DB{self.db_number} starting at offset {start}: {real_list}"
        )
        ...
        for i, value in enumerate(real_list):
            ...
            if write_result:
                ...
                self.get_logger().debug(
                    f"Failed to write REAL value {value} at offset {offset} (index {i})"
                )
        ...
        self.get_logger().debug(
            f"Successfully wrote {success_count}/{len(real_list)} REAL values starting at offset {start}"
        )
    ...
    def write_registers_uint16(self, reg_list, start=64):
        ...
        if not reg_list:
            self.get_logger().warning(
                f"Empty register list provided for write at offset {start}"
            )
            return False
        ...
        # 记录发送的数据日志，便于对比发送的数据和接收的数据是否一致
        self.get_logger().info(
            f"[LOG] Sending {len(reg_list)} registers to DB{self.db_number} starting at offset {start}: {reg_list}"
        )
        ...
        for i, value in enumerate(reg_list):
            ...
            if write_result:
                ...
                self.get_logger().debug(
                    f"Failed to write value {converted_value} at offset {offset} (index {i})"
                )
                # Check if this was a CPU access error and stop further writes if so
                # This prevents spamming the PLC with requests it will refuse
                # We can continue with other operations though
        ...
        self.get_logger().debug(
            f"Successfully wrote {success_count}/{len(reg_list)} registers starting at offset {start}"
        )
        # Return True even if some writes failed, as long as we attempted them all
        # This allows the program to continue functioning
    ...
    def cam_bringup(self):
        ...
        if self.cam_proc and self.cam_proc.poll() is None:
            self.get_logger().info(
                "Terminating existing camera process before starting new one..."
            )
            try:
                ...
                try:
                    ...
                    self.cam_proc.kill()
                    self.get_logger().info(
                        "Killed existing camera process that did not terminate gracefully"
                    )
            except Exception as e:
                self.get_logger().warning(
                    f"Error terminating existing camera process: {e}"
                )
        ...
        for attempt in range(max_attempts):
            self.get_logger().info(
                f"Camera startup attempt {attempt + 1}/{max_attempts}"
            )

            # 每次尝试时都检查物理连接
            if not self.check_physical_camera_connection_stable():
                self.get_logger().info(
                    f"Physical camera not connected on attempt {attempt + 1}, waiting for connection..."
                )
                # 在等待一段时间后再次尝试
                time.sleep(2)
                continue
            ...
            try:
                ...
                # 构造命令： ros2 run snap_7 custom_realsense_node
                ...
                if self.cam_proc.poll() is not None:
                    self.get_logger().error(
                        f"Camera process exited immediately with code {self.cam_proc.poll()} on attempt {attempt + 1}"
                    )
                    self.camStatus = False
                    # 终止进程并准备重试
                    try:
                        if self.cam_proc and self.cam_proc.poll() is None:
                            ...
                            try:
                                ...
                                self.cam_proc.kill()
                                self.get_logger().info(
                                    f"Killed failed camera process on attempt {attempt + 1}"
                                )
                    except:
                        pass
                    # 在重试前稍等
                ...
                # 添加相机状态监控
                if (
                    hasattr(self, "camera_monitor_thread")
                ...
                self.get_logger().error(
                    f"Failed to start camera process on attempt {attempt + 1}: {e}"
                )
                self.camStatus = False  # 确保将状态设为False，以便后续可以重试
                # 终止进程并准备重试
                try:
        ...
        if not self.camStatus:
            self.get_logger().error(
                f"Failed to start camera after {max_attempts} attempts"
            )
            self.write_registers_uint16([999], start=16)  # 发送错误代码
        ...
        while self.camStatus and rclpy.ok():
            try:
                ...
                if not self.check_physical_camera_connection_stable():
                    ...
                    self.get_logger().warning(
                        f"Physical camera disconnected (count: {self.camera_disconnect_count})"
                    )

                    # 检查是否在宽限期内
                    time_since_connect = (
                        time.time() - start_time
                        if hasattr(self, "camera_connect_time")
                    ...
                    if self.camera_disconnect_count >= threshold:
                        self.get_logger().warning(
                            f"Physical camera disconnected permanently after {self.camera_disconnect_count} consecutive failures, stopping camera"
                        )
                        # 设置相机状态为断开
                        self.camStatus = False
                        # 向PLC发送错误状态
                ...
                if self.cam_proc and self.cam_proc.poll() is not None:
                    # 相机进程意外退出
                    self.get_logger().warning(
                        f"Camera process unexpectedly terminated with code {self.cam_proc.poll()}"
                    )
                    self.camStatus = False
                    # 向PLC发送错误状态
                    self.write_registers_uint16([999], start=16)  # 发送错误代码
    ...
    def wait_for_camera_ready(self, timeout=15):
        ...
        while time.time() - start_time < timeout:
            if (
                ...
                and self.check_physical_camera_connection()
            ):
                self.get_logger().info(
                    "Camera process and physical connection OK, waiting for initialization..."
                )
                ...
                if (
                    ...
                    self.get_logger().info(
                        "Camera connection lost during initialization wait"
                    )
            time.sleep(0.5)
        return False
    ...
    def check_physical_camera_connection(self):
        ...
        try:
                ...
                if len(devices) > 0:
                    ...
                    for device in devices:
                        try:
                            ...
                            self.get_logger().debug(
                                f"Physical RealSense Camera Found: {device_name}, Serial: {serial_number}"
                            )
                            return True
                        except Exception:
                            continue
            ...
            try:
                ...
                self.get_logger().warning(
                    f"Error checking physical camera connection: {e}"
                )
                return False
        except Exception as e:
    ...
    def safe_camera_operation(self):
        """安全的相机操作，包含连接检查"""
        if (
            not self.check_camera_connection()
            or not self.check_physical_camera_connection_stable()
        ):
            self.get_logger().warning(
                "Camera not connected or physically disconnected, attempting to restart..."
            )
            self.cam_bringup()
            time.sleep(3)  # 等待相机启动
            if (
    ...
    def norm_bringup(self, seq):
        ...
        if not self.camStatus:
            self.get_logger().warning(
                "Camera not launched yet, cannot perform norm calculation"
            )
            # 向PLC发送错误状态
        ...
        if not self.check_physical_camera_connection():
            self.get_logger().warning(
                "Physical camera not connected, cannot perform norm calculation"
            )
            # 向PLC发送错误状态
            self.write_registers_uint16([998], start=16)  # 相机未启动错误代码
            return
    ...
    def _handle_norm_response(self, resp, seq):
            ...
            for i, pose in enumerate(resp.pose_list.poses):
                if i < 24:  # Only process first 24 poses
                    ...
                    vals = [
                        ...
                        clamped_euler_rad[0],
                        clamped_euler_rad[1],
                        clamped_euler_rad[2],  # A(Z), B(Y), C(X) 角度（弧度）
                    ]
        ...
        self.get_logger().debug(
            f"Attempting to write {len(output_data)} REAL values to offset 20"
        )
        ...
        self.get_logger().debug(
            f"Attempting to write point count {pointNum} to offset 18"
        )
        pointnum_write_success = self.write_registers_uint16(
            [int(pointNum) & 0xFFFF], start=18
        )
        ...
        success_count = sum(
            [pose_write_success, pointnum_write_success, rosstatus_write_success]
        )
        total_ops = 3
        self.get_logger().info(
            f"Norm calculation done. {min(len(resp.pose_list.poses), 24)} poses processed, {success_count}/{total_ops} write operations successful"
        )

        if not (
            pose_write_success and pointnum_write_success and rosstatus_write_success
        ):
            self.get_logger().warning(
                "Some write operations failed. This may be due to PLC DB configuration or write protection."
            )
    ...
    def poll_plc(self):
        ...
        if plcStatus != self.prev_trigger:
            self.get_logger().info(
                f"plcStatus changed: {self.prev_trigger} -> {plcStatus}"
            )
            ...
            if self.Trigger_Portal == 100 and self.Old_trigger != self.Trigger_Portal:
                ...
                if not self.check_physical_camera_connection_stable():
                    self.get_logger().warning(
                        "Physical camera not connected, attempting to start camera anyway..."
                    )
                    # 即使物理连接不可用，也要尝试启动相机，因为用户可能刚刚插入了相机
                    self.cam_bringup()
                else:
                    self.get_logger().info(
                        "Physical camera detected, proceeding to start camera"
                    )
                    if not self.camStatus:
                        self.cam_bringup()
                ...
                self.get_logger().info(
                    f"Camera process status: {self.check_camera_connection()}"
                )
                self.get_logger().info(
                    f"Physical camera status: {self.check_physical_camera_connection_stable()}"
                )
                ...
                if is_ready:
                    ...
                    self.get_logger().error(
                        "Failed to launch camera or camera not connected"
                    )
                ...
                if self.safe_camera_operation():
                    ...
                    self.get_logger().error(
                        "Cannot perform norm calculation: camera not connected"
                    )
                    # 向PLC发送错误状态
                ...
                self.Trigger_Portal == 130
                or 120
                and self.Old_trigger != self.Trigger_Portal
            ):
