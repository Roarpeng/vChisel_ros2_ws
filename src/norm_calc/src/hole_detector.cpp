...
bool holeDetector(cv::Mat src, cv::Mat depthInfo, sensor_msgs::msg::CameraInfo camInfo, pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_holes)
{
    ...
    if (centerLists.size() > 0)
    {
        ...
        for (int i = 0; i < centerLists.size(); i++)
        {
            ...
            cloud_holes->points[i].x = (centerLists[i].x - camInfo.k[2])/camInfo.k[0] * cloud_holes->points[i].z;
            cloud_holes->points[i].y = (centerLists[i].y - camInfo.k[5])/camInfo.k[4] * cloud_holes->points[i].z;
