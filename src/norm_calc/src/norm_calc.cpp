              ...
              edge_grid::GridParam gridParam)
{
  ...
  pass.setFilterLimits (chiselParam.ZMIN, chiselParam.ZMAX);        //设置在过滤字段的范围
  ...
  pass.setFilterLimits (chiselParam.YMIN - chiselParam.BORDER_WIDTH, chiselParam.YMAX + chiselParam.BORDER_WIDTH);        //设置在过滤字段的范围
  // pass.setFilterLimitsNegative (true);   //设置保留范围内还是过滤掉范围内
  pass.filter (*cloud_filtered);

  pass.setInputCloud (cloud_filtered);      //设置输入点云
  pass.setFilterFieldName ("x");            //设置过滤时所需要点云类型的x字段
  pass.setFilterLimits (chiselParam.XMIN - chiselParam.BORDER_WIDTH, chiselParam.XMAX + chiselParam.BORDER_WIDTH);        //设置在过滤字段的范围
  // pass.setFilterLimitsNegative (true);   //设置保留范围内还是过滤掉范围内
  pass.filter (*cloud_filtered);
  ...
  #ifdef USING_SMOOTH
  pcl::concatenateFields (*cloud_smoothed, *cloud_normals, *cloud_with_normals);    //连接字段
  #else
  pcl::concatenateFields (*cloud_downSampled, *cloud_normals, *cloud_with_normals);    //连接字段
  #endif

  // ********三角化start********
  // //定义搜索树对象
  // pcl::search::KdTree<pcl::PointXYZRGBNormal>::Ptr treeTriangle (new pcl::search::KdTree<pcl::PointXYZRGBNormal>);
  // // Initialize objects
  ...
  // gp3.setSearchRadius (0.15);                                       //设置连接点之间的最大距离，（即是三角形最大边长）
  // // 设置各参数值
  ...
  holePass.setFilterLimits (chiselParam.ZMIN, chiselParam.ZMAX);        //设置在过滤字段的范围
  holePass.filter (*cloud_holes);

  holePass.setInputCloud (cloud_holes);      //设置输入点云
  holePass.setFilterFieldName ("y");            //设置过滤时所需要点云类型的y字段
  holePass.setFilterLimits (chiselParam.YMIN - chiselParam.BORDER_WIDTH, chiselParam.YMAX + chiselParam.BORDER_WIDTH);        //设置在过滤字段的范围
  holePass.filter (*cloud_holes);

  holePass.setInputCloud (cloud_holes);      //设置输入点云
  holePass.setFilterFieldName ("x");            //设置过滤时所需要点云类型的x字段
  holePass.setFilterLimits (chiselParam.XMIN - chiselParam.BORDER_WIDTH, chiselParam.XMAX + chiselParam.BORDER_WIDTH);        //设置在过滤字段的范围
  holePass.filter (*cloud_holes);
  ...
  for (size_t i = 0; i < EDGE_ROW; i++)
  {
    ...
    {
      edgeTable[i][j] = new edge_grid::EdgeGrid(i, j,gridParam.THDEEP, gridParam.THDEEPNORM, gridParam.THHALF, gridParam.THFULL,gridParam.THANGLE);
    }
  }
  int indRow,indColumn;
  for (size_t i = 0; i < cloud_shrink->points.size(); i++) // 将符合条件的法向点归纳至20*10栅格区域类
  {
    indRow    = floor((cloud_shrink->points[i].y - gridParam.YMIN) / gridParam.GRID_LEN);
    indColumn = floor((cloud_shrink->points[i].x - gridParam.XMIN) / gridParam.GRID_LEN);
    if (0 <= indRow && indRow < gridParam.GRID_ROW && 0<= indColumn && indColumn < gridParam.GRID_COLUMN)
    {
      if (!edgeTable[indRow][indColumn]->addPointInd(i))
      {
        cout << "add point err! " << i << "," <<indRow << "," << indColumn << endl;
          ...
          gridNormTable[ii][jj].status == edge_grid::SLANT)
      {
        ...
        switch (gridNormTable[ii][jj-1].status)
        {
          ...
          case edge_grid::DEEP:
            ...
            {
              tarPointList[tarIndex].ox   = gridNormTable[ii][jj].ox + 0.15f * gridParam.GRID_LEN;
            }
            else
            {
              tarPointList[tarIndex].ox   = gridNormTable[ii][jj].ox; // - 0.15f * gridParam.GRID_LEN;
          ...
          case edge_grid::CRACK_FLAT:
            tarPointList[tarIndex].ox     = gridNormTable[ii][jj].ox - 0.05f * gridParam.GRID_LEN;
            break;
          default:
            break;
        } 
  ...
  for (size_t i = 0; i < cloud_shrink->points.size(); i++) // 将符合条件的法向点归纳至4*6凿击区域类
  {
    ...
    else if (chiselParam.BOX_ROW <= fRow &&  fRow <= 0.3f + chiselParam.BOX_ROW)
    {
    ...
    else if (chiselParam.BOX_COLUMN <= fCol &&  fCol <= 0.3f + chiselParam.BOX_COLUMN)
    {
    ...
    if (0 <= indRow && indRow < chiselParam.BOX_ROW && 0<= indColumn && indColumn < chiselParam.BOX_COLUMN)
    {
