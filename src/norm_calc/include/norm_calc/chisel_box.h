...
// #define ZMIN          0.2f
// #define ZMAX          0.6f
// #define BORDER_WIDTH  0.02f   // 可凿击区域外延伸的边界宽度，用于更好计算可凿击边界法向量
// #define NORM_TH       0.9f   // 可凿击法向量z阈值 acos(0.9) = 25.8degree
// #define AFFECT_RADIUS 0.0009f // 凿击影响区域半径平方，0.03*0.03
// #define HEIGHT_WEIGHT 3.0f    // 选取凿击法向量投票函数中高度的权重
// #define CURV_WEIGHT   2.0f   // 选取凿击法向量投票函数中曲率的权重
// #define ANGLE_WEIGHT  1.0f   // 选取凿击法向量投票函数中jiaodu的权重
// #define SEARCH_RADIUS 0.03f   // 邻域统计半径
// #define SEARCH_NUM_TH 100     // 邻域统计数量阈值
...
typedef struct
{
  ...
  float ZMIN;
  float ZMAX;
  float BORDER_WIDTH;   // 可凿击区域外延伸的边界宽度，用于更好计算可凿击边界法向量
...
class ChiselBox
{
  ...
  bool voteTarPoint(pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr cloud,pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_holes,size_t row, size_t column, ChiselNormPoint *tarPointList);
  bool getTarStatus();
